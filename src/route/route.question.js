...
questionRoute.route("/question").post((req, res, next) => {
  ...
  addToDistributionCollection(UserId);
});
...
async function addToDistributionCollection(UserId) {
  ...
  distribution.findOneAndUpdate(
    { userId: UserId, RangeMax: 1, RangeMin: 1 },
    {
      $inc: { countQuestion: 1 },
    },
    (error) => {
...
questionRoute.route("/question/quiz").get(async (req, res, next) => {
  ...
  distribution.find({ userId: usrId }, (error, data) => {
    if (error) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        quiz
          .find({
            userId: usrId,
            expertiseLevel: { $gt: temp.RangeMin, $lte: temp.RangeMax },
          })
          .limit(equity)
...
function deleteFromQuestion(qId) {
  question.deleteOne({ questionId: qId }, (error, next) => {
    if (error) {
      ...
      console.log(
        "Question with QuesId { " +
          qId +
          " }deleted Successfully from Question Collection  "
      );
    }
  });
}
      ...
      distribution.findOneAndUpdate(
        ...
        (error, countQue) => {
          if (error) {
            ...
            console.log(
              "Error: Total number of Question is negative:: " +
                countQue.countQuestion
            );
            return next(error);
          } else {
            console.log(
              "After updation Value of CountQuestion is:" +
                countQue.countQuestion
            );
          }
        }
...
questionRoute.route("/question").get(async (req, res) => {
  const arr = [];
  quiz.find({ userId: req.body.userId }).then(async (data) => {
    for (let i = 0; i < data.length; i++) {
...
//Quiz submit api
questionRoute.route("/question/quiz/submit").get((req, res, next) => {
  var qId = req.body.questionId;
  var usrId = req.body.userId;
  for (let i = 0; i < qId.length; i++) {
    quiz
      .findOne({ questionId: qId[i], userId: usrId })
      .then(async (expLevel) => {
        let exp = expLevel.expertiseLevel;
        // Storing Duplicate Value of explevel
        let dupexp = exp;
        if (req.body.response[i] == "Yes") {
          // finding if value is power of two or not
          if (powerOfTwo(exp)) {
            exp = 2 * exp;
          } else {
            exp = (exp + nextPower(exp)) / 2;
          }
        } else {
          exp = (3 * exp) / 4;
        }
        ...
        // updating countQuestion for the range
        findandUpdate(usrId, dupexp, -1);
        // finding total count for the User
        let count = await countDocument(usrId);
        if (count == 0) {
          //creating new document for the exp value
          distribution.create(
            { userId: usrId, RangeMax: nextPower(exp), RangeMin: exp },
            (error) => {
              if (error) {
                console.log("Error: Failed to create" + error);
                return next(error);
              }
            }
          );
        ...
        //updating expertise level in quiz Collection
        quiz.findOneAndUpdate(
          { questionId: qId[i], userId: usrId },
          { expertiseLevel: exp },
          (error) => {
            if (error) {
              console.log("Failed to find and update::" + error);
              ...
              console.log("After Updation ExpertiseLevel Value:" + exp);
              console.log("ExpertiseLevel updated successfully!");
            }
          }
        );
      })
      .catch((error) => {
        if (error) {
          console.log("Failed to find and update in Quiz Collection ::" + error);
          return next(error);
        }
      });
  }
...
async function countDocument(UserId) {
  return distribution.countDocuments({ userId: UserId }).then((count) => {
    return count;
  });
}
// update in Distribution Collection
function findandUpdate(usrId, dupexp, val) {
  distribution.findOneAndUpdate(
    {
      userId: usrId,
      RangeMin: { $lt: dupexp },
...
function powerOfTwo(exp) {
  return (exp && exp & (exp - 1)) == 0;
}
// finding next power of the expertise value 
function nextPower(exp) {
  let power = 1;
  while (power < exp) power *= 2;
  return power;
}
